...
Usage:
    # From terminal log
    sudo python3 src/power_predictor.py --model checkpoints/tau8/best_model.pth --live --scroll 2>&1 | tee prediction_log.txt
    python3 scripts/plot_predictions.py prediction_log.txt

    # From CSV file
    python3 src/power_predictor.py --model checkpoints/tau8/best_model.pth --live --save predictions.csv
...
def parse_prediction_line(line):
    """
    Parse a line like:
    [19:16:18] #   344 (  1.0Hz) | CPU:  65.6% | Pred:  69.23W | Actual:  60.08W | Err: +15.24% | MAPE:  9.03%

    Returns: dict with parsed data or None if line doesn't match
    """
    # Pattern to match the log format
    pattern = r'\[(\d+:\d+:\d+)\]\s+#\s+(\d+)\s+\([^)]+\)\s+\|\s+CPU:\s+([\d.]+)%\s+\|\s+Pred:\s+([\d.]+)W\s+\|\s+Actual:\s+([\d.]+)W\s+\|\s+Err:\s+([+-][\d.]+)%'
...
def parse_csv_file(file_path):
    ...
    Expected columns:
    - timestamp, sample, cpu_usage, predicted_power, actual_power, error_pct
    ...
    required = ['sample', 'cpu_usage', 'predicted_power']
    ...
    for _, row in df.iterrows():
        # Parse cpu_usage - could be numeric or dict string
        cpu_usage_raw = row['cpu_usage']
        if isinstance(cpu_usage_raw, str) and cpu_usage_raw.startswith('{'):
            # Multi-feature format: "{'user_percent': 0.0, 'system_percent': 0.0, ...}"
            try:
                import ast
                cpu_dict = ast.literal_eval(cpu_usage_raw)
                # Extract total CPU (user + system)
                cpu_usage = cpu_dict['user_percent'] + cpu_dict['system_percent']
            except:
                # If parsing fails, default to 0
                cpu_usage = 0.0
        else:
            # Single-feature format: numeric value
...
def plot_predictions(data, output_file='results/plots/prediction_comparison.png'):
    ...
    if has_actual:
        ...
        fig.suptitle('DECODE-RAPL Power Prediction Analysis', fontsize=16, fontweight='bold')
        ...
        fig.suptitle('DECODE-RAPL Power Predictions', fontsize=16, fontweight='bold')
...
def main():
    ...
    if not data:
        ...
        print("\nExpected formats:")
        print("  Log: [HH:MM:SS] #   N (1.0Hz) | CPU: XX.X% | Pred: XX.XXW | Actual: XX.XXW | Err: +XX.XX% | MAPE: XX.XX%")
        print("  CSV: timestamp,sample,cpu_usage,predicted_power,actual_power,error_pct")
        sys.exit(1)
